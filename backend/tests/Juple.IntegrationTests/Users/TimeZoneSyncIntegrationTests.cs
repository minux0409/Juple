...
/// <summary>
/// Covers CurrentUserProvisioningStore.TrySyncTimeZoneIfExistingAsync - the fix for an existing
...
public sealed class TimeZoneSyncIntegrationTests : IAsyncLifetime
{
    ...
    private async Task<ExternalIdentityPrincipal> SeedExistingUserAsync(string initialTimeZoneId)
    {
        ...
        await store.CreateOrGetAsync(new CurrentUserBootstrapData(
            externalIdentity,
            "en-US",
            initialTimeZoneId,
    ...
    [Fact]
    public async Task TrySyncTimeZoneIfExistingAsync_WhenStoredTimeZoneDiffers_UpdatesToTheDeviceTimeZone()
    {
        ...
        var handled = await store.TrySyncTimeZoneIfExistingAsync(externalIdentity, "Asia/Seoul", updatedAtUtc);
        _dbContext.ChangeTracker.Clear();
    ...
    [Fact]
    public async Task TrySyncTimeZoneIfExistingAsync_WhenStoredTimeZoneAlreadyMatches_DoesNotWrite()
    {
        ...
        var handled = await store.TrySyncTimeZoneIfExistingAsync(
    ...
    [Fact]
    public async Task TrySyncTimeZoneIfExistingAsync_WhenIdentityDoesNotExist_ReturnsFalseAndWritesNothing()
    {
