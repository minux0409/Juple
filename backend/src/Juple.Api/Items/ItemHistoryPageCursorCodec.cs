...
/// <summary>
/// Encodes/decodes the opaque `cursor` query value for GET /api/v1/items/history. Callers below
/// the API layer (Application, Store) work only with the typed <see cref="ItemHistoryPageCursor"/>
/// - they never see this Base64/JSON wire format. The payload carries no UserId, URL, token, or
/// other PII; it is not an authorization mechanism.
/// </summary>
public static class ItemHistoryPageCursorCodec
{
    ...
    public static bool TryDecode(string value, out ItemHistoryPageCursor? cursor)
    {
        cursor = null;
